<template>
  <div class="col-lg-8">
      ...
      <div class="pl-content">
        <div class="input-group">
          <input
            ...
            :placeholder="$t('followings.search')"
            @keydown="
              $router.push({
        ...
        <div class="alert alert-ino">
          ...
          <nuxt-link
            ...
            :to="localePath('/users/')"
            :title="$t('users.users')"
            >{{ $t("followings.clickHereToFindUsersToFollow") }}</nuxt-link
          >
        </div>
      </div>
    </div>
    <div
      ...
      v-if="myFollowings"
    >
      <div class="col-sm-4 mb-3" v-for="item in myFollowings" :key="item.id">
        <div class="pl-box h-100">
          ...
          <div class="pl-content pb-3">
            ...
            <button
              ...
              @click.prevent="
                followUnfollow(item.is_following, item.following_id)
              "
      ...
      <div class="col-sm-12 mt-3" v-if="recordsFiltered > myFollowings.length">
          ...
          <ul class="pagination justify-content-center text-center">
            <li :class="['page-item', { disabled: currentPage === 1 }]">
              <a
                ...
                title="Önceki Sayfa"
                class="page-link"
                @click.prevent="(currentPage -= 1), getMyFollowings()"
            ...
            >
              <a
                ...
                title="Sonraki Sayfa"
                class="page-link"
                @click="(currentPage += 1), getMyFollowings()"
                href="javascript:void(0)"
                aria-label="Next"
              >
                <span
      ...
      <div class="pl-content">
        <div class="alert alert-danger">
          <div class="text-center">
            ...
            <h5 class="text-blue mb-0">{{ $t("followings.notFound") }}</h5>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit("SET_BREADCRUMB", {
      showBreadCrumb: true,
      breadCrumbTitle: this.$t("profile.myFollowings"),
      breadCrumbDescription: this.$t("profile.myFollowings"),
    });
    this.$nextTick(() => {
  ...
  methods: {
    ...
    async getMyFollowings() {
      try {
        const { data } = await this.$axios.get("my-followings", {
          params: {
  ...
  head() {
    return {
      ...
      title: this.$t("profile.myFollowings"),
    };
  },
};
</script>