<template>
  <div>
    <label for="position" class="mb-5">{{ $t("panel.menus.position") }}</label>
    <select
      name="position"
      id="position"
      class="form-control form-control-sm rounded-0 mb-3"
      v-model="menuType"
      required
      @change.prevent="getMenus()"
    >
    ...
    >
      <template v-slot="{ node, tree }">
        <div
          class="mb-1 bg-gold-light-5 d-flex align-items-center align-self-center align-content-center p-2"
        >
          <b
          ...
          <input
            ...
            class="mx-15"
          />
          <small class="font-weight-bold">#ID : {{ node.id }}</small>

          <span> - {{ node.title }}</span>
          <span class="form-check form-switch ml-15">
            <input
          ...
          <nuxt-link
            ...
            :to="localePath('/panel/menus/update/' + node.id)"
  ...
  methods: {
    ...
    checkAll() {
      this.menus.forEach((el) => {
        if (!this.checked_menus.includes(el)) this.checked_menus.push(el);
      });
    },
    // Rank Change
    ...
    async getMenus() {
      try {
        let { data } = await this.$axios.get(
          "panel/menus/?menutype=" + this.menuType
        );
        if (data && data.data) {
