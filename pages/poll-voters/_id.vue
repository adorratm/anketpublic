  ...
  methods: {
    ...
    async getPoll() {
      try {
        ...
        this.poll = data.data[0];
        this.$store.commit("SET_SLIDER", false);
        this.$store.commit("SET_BREADCRUMB", {
          showBreadCrumb: true,
          breadCrumbTitle: this.poll.title + " - " + this.$t("poll.pollVoters"),
          breadCrumbDescription:
            this.poll.title + " - " + this.$t("poll.pollVoters"),
        });
      } catch (error) {
        console.log(error);
      }
    ...
    async getUsers() {
      try {
        const { data } = await this.$axios.get(
          "poll-voters/" + this.$route.params.id,
          {
            params: {
