          ...
          <span v-else-if="props.column.field === 'isActive'">
            <div class="form-check form-switch">
              <input
                ...
                type="checkbox"
                class="my-check form-check-input float-none"
                :checked="props.row.isActive == 1 ? true : false"
                @input.prevent="changeIsActive($event, props.row.id)"
              />
            </div>
          </span>
          <span v-else-if="props.column.field === 'isPopular'">
            <div class="form-check form-switch">
              <input
                :id="'customSwitch' + props.row.id"
                type="checkbox"
                class="my-check form-check-input float-none"
                :checked="props.row.isPopular == 1 ? true : false"
                @input.prevent="changeIsPopular($event, props.row.id)"
              />
            </div>
          </span>
          <span v-else-if="props.column.field === 'pinned'">
            <div class="form-check form-switch">
              <input
                ...
                type="checkbox"
                class="my-check form-check-input float-none"
                :checked="props.row.pinned == 1 ? true : false"
                @input.prevent="changePinned($event, props.row.id)"
              />
            </div>
          </span>
            ...
            <div class="dropdown">
              ...
              >
                <nuxt-link
                  v-if="editurl"
                  rel="dofollow"
                  :title="$t('panel.editRecord')"
                  class="dropdown-item"
                  :to="localePath(editurl + props.row.id)"
                ...
                <div v-if="customActions?.length">
                  <nuxt-link
                    ...
                    :title="action.label"
                    class="dropdown-item"
                    :to="localePath(action.url + props.row.id)"
                    ><i :class="action.icon"></i>{{ action.label }}</nuxt-link
                  >
                </div>
                <a
                  v-if="deleteurl"
                  rel="dofollow"
                  :title="$t('panel.deleteRecord')"
  ...
  methods: {
    checkAll() {
      this.rows.forEach((el) => {
        if (!this.checkeds.includes(el.id)) this.checkeds.push(el.id);
      });
    },
    ...
    // Popular Change
    async changeIsPopular(e, id) {
      try {
        let response = await this.$axios.$put(this.ispopularurl + id, {
    ...
    // Pinned Change
    async changePinned(e, id) {
      try {
        let response = await this.$axios.$put(this.pinnedurl + id, {
