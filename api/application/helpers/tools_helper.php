...
function send_emailv2($toEmail = [], $subject = null, $message = null, $attachments = [], $lang = "tr", $mail_settings = 1)
{
    ...
    $email_settings->user_name = !empty($email_settings->email) ? $email_settings->email : null;
    $email_settings->user = !empty($email_settings->email) ? $email_settings->email : null;
    $email_settings->password = !empty($email_settings->password) ? $email_settings->password : null;
    $email_settings->from = !empty($email_settings->email) ? $email_settings->email : null;
    $email_settings->to = !empty($email_settings->email) ? $email_settings->email : null;
    ...
    if (empty($toEmail)) :
        $toEmail = [$email_settings->email];
    endif;
    // Create a message
    $msg = (@new Swift_Message($subject))
        ->setFrom([$email_settings->email => $email_settings->email])
        ->setTo($toEmail)
        ->setCharset('utf-8');
...
function guzzle_request($url = null, $port = null, $endpoint = null, $data = [], $header = ["Content-Type" => "application/json", "Accept" => "application/json"])
{
    try {
        ...
        $client = new Client(['decode_content' => false,]);
        if (!empty($data)) {
            if (is_array($data)) :
                $data = json_encode($data);
            endif;
            $request = new Request('POST', $url, $header, $data);
        } else {
...
function check_level($role_id = null)
{
    // 1 = super admin, 2 = admin, 3 = user, 4 = moderator 
    if ($role_id != 1) {
        return true;
    }
    return false;
...
function is_voted($poll_id = null, $user_id = null, $author_id = null)
{
    $t = &get_instance();
    if ($t->general_model->get("votes", null, ["poll_id" => $poll_id, "user_id" => $user_id, "author_id" => $author_id])) {
        return true;
    } elseif (check_ip() && $t->general_model->get("votes", null, ["poll_id" => $poll_id, "user_id" => $user_id, "author_id" => $author_id, "ip" => check_ip()])) {
        return true;
    } else {
        return false;
    }
...
function calculate_age($birth_date = null)
{
...
function fh_stats_sex($poll_id, $votes, $gender)
{
    $t = &get_instance();
    $m = $t->general_model->rowCount("votes", ["poll_id" => $poll_id, "gender" => $gender]);
