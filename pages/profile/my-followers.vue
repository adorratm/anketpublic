<template>
  <div class="col-lg-8">
    ...
    <div
      ...
      v-if="myFollowers"
    >
      <div class="col-sm-4 mb-3" v-for="item in myFollowers" :key="item.id">
      ...
      <div class="col-sm-12 mt-3" v-if="recordsFiltered > myFollowers.length">
          ...
          <ul class="pagination justify-content-center text-center">
            <li :class="['page-item', { disabled: currentPage === 1 }]">
              <a
                ...
                title="Önceki Sayfa"
                class="page-link"
                @click.prevent="(currentPage -= 1), getMyFollowers()"
            ...
            >
              <a
                ...
                title="Sonraki Sayfa"
                class="page-link"
                @click="(currentPage += 1), getMyFollowers()"
                href="javascript:void(0)"
                aria-label="Next"
              >
                <span
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit("SET_BREADCRUMB", {
      showBreadCrumb: true,
      breadCrumbTitle: this.$t("profile.myFollowers"),
      breadCrumbDescription: this.$t("profile.myFollowers"),
    });
    this.$nextTick(() => {
  ...
  methods: {
    ...
    async getMyFollowers() {
      try {
        const { data } = await this.$axios.get("my-followers", {
          params: {
  ...
  head() {
    return {
      ...
      title: this.$t("profile.myFollowers"),
    };
  },
};
</script>