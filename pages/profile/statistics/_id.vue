      ...
      <div class="pl-cpanel-heading">
        <div class="pl-row">
          <div class="pl-col-9 pl-cpanel-title">
            <h3><i class="icon-chart icons"></i>{{ $t("statistics.statistics") }}</h3>
            <p>
          ...
          <div class="pl-col-3 text-right">
            <a
              ...
              ><i class="fas fa-file-pdf"></i> {{ $t("statistics.exportToPdf") }}</a
            >
          </div>
        </div>
      </div>
      <div class="pl-box p-4" id="root">
        <div>
          ...
          <p class="pl-chart-content">
            <i class="fa fa-thumbs-up"></i> {{ poll.votes_count }} {{ $t("totalVotes") }}
            <i class="fa fa-comments ml-3"></i> {{ poll.comments_count }}
            {{ $t("totalComments") }}
          </p>
        </div>
        ...
        <div v-if="poll.answers.length">
          <div class="pt-answer-stats" v-for="answer in poll.answers" :key="answer.id">
            <div class="mb-3">
              <b class="d-block">{{
              ...
              <img
                ...
                :data-src="$config.UPLOADS_URL + '/polls/answers/' + answer.thumbnail"
                width="300"
                :alt="
                  parseInt(poll.polltype) === 2
                    ? $t('poll.' + answer.answer)
              ...
              <div class="pt-l1">
                <span
                  class="pt-l2 d-block"
                  :style="
                    'width: ' + answer.percentage + '%; background: #' + answer.color.hex
                  "
                >
                  <small
                    class="px-3 fw-bold"
                ...
                <table class="table table-bordered">
                  <tbody>
                    <tr>
                      ...
                      <td width="15%" class="text-center">{{ poll.genders.male }}%</td>
                    </tr>
                    <tr>
                      <td width="85%">{{ $t("register.female") }}</td>
                      <td width="15%" class="text-center">{{ poll.genders.female }}%</td>
                    </tr>
                    <tr>
                      <td width="85%">{{ $t("register.other") }}</td>
                      <td width="15%" class="text-center">{{ poll.genders.other }}%</td>
                    </tr>
                  </tbody>
                </table>
              </td>
        ...
        <table class="table table-bordered" v-if="poll.cities_and_districts.length">
          <tbody>
            <tr
          ...
          <table class="table border">
            ...
            <tbody>
              <tr v-for="voter in searchResults" :key="voter.id">
                <td width="40%" class="text-left">
                  ...
                  >
                    <a
                      ...
                      :title="maskName(voter.voter_username)"
                    >
                      <img
                        v-if="voter.voter_photo"
                        loading="lazy"
                        class="lazyload img-fluid"
                        :data-src="$config.UPLOADS_URL + '/users/' + voter.voter_photo"
                        :alt="maskName(voter.voter_username)"
                        width="100"
                      />
                    ...
                    <a
                      ...
                      :title="maskName(voter.voter_username)"
                    >
                      {{ maskName(voter.voter_username) }}</a
                    >
                  </div>
                ...
                <td width="20%" class="text-center text-nowrap small align-middle">
                  {{ voter.createdAt }}
                </td>
              </tr>
            </tbody>
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit("SET_BREADCRUMB", {
      showBreadCrumb: true,
      breadCrumbTitle: this.$t("statistics.statistics"),
      breadCrumbDescription: this.$t("statistics.statistics"),
    });
    this.$nextTick(() => {
      ...
      setTimeout(() => {
        ...
        if ($("#question-age").length) {
          ...
          $.pieChart("question-age", agesDataLabelss, agesDataCnts, agesDataClrs);
        }
      }, 500);
    });
  },
  methods: {
    maskName(value) {
      ...
      // Mask the first part (assuming it's a single character)
      const maskedFirstPart = parts[0].charAt(0) + "*".repeat(parts[0].length - 1);
      // Mask the rest of the parts
      const maskedRest = parts.slice(1).map((part) => "***");
      // Join the masked parts and return
      return maskedFirstPart + " " + maskedRest.join(" ");
    },
    filterSearch() {
      this.searchResults = this.poll.voters.filter((voter) => {
        return (
          voter?.voter_username?.toLowerCase()?.includes(this?.search?.toLowerCase()) ||
          voter?.city?.toLowerCase()?.includes(this.search?.toLowerCase()) ||
          voter?.district?.toLowerCase()?.includes(this.search?.toLowerCase()) ||
    ...
    async getMyPoll() {
      try {
        const { data } = await this.$axios.get("/my-poll/" + this.$route.params.id);
        this.poll = data.data;
