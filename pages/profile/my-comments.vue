<template>
  <div class="col-lg-8">
    ...
    <div
      ...
      v-if="myComments"
    >
      <div class="col-sm-12 mb-3" v-for="item in myComments" :key="item.id">
      ...
      <div class="col-sm-12 mt-3" v-if="recordsFiltered > myComments.length">
          ...
          <ul class="pagination justify-content-center text-center">
            <li :class="['page-item', { disabled: currentPage === 1 }]">
              <a
                ...
                title="Önceki Sayfa"
                class="page-link"
                @click.prevent="(currentPage -= 1), getMyComments()"
            ...
            >
              <a
                ...
                title="Sonraki Sayfa"
                class="page-link"
                @click="(currentPage += 1), getMyComments()"
                href="javascript:void(0)"
                aria-label="Next"
              >
                <span
...
export default {
  ...
  mounted() {
    ...
    this.$store.commit("SET_BREADCRUMB", {
      showBreadCrumb: true,
      breadCrumbTitle: this.$t("profile.myComments"),
      breadCrumbDescription: this.$t("profile.myComments"),
    });
    this.$nextTick(() => {
  ...
  methods: {
    async deleteComment(id) {
      try {
        const { data } = await this.$axios.delete("delete-my-comment/" + id);
        if (data.status) {
    ...
    async getMyComments() {
      try {
        const { data } = await this.$axios.get("my-comments", {
          params: {
  ...
  head() {
    return {
      ...
      title: this.$t("profile.myComments"),
    };
  },
};
</script>