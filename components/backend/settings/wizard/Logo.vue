  ...
  <div class="row mb-3">
    ...
    <div class="col-sm-4">
      <div class="form-group my-3">
        <ValidationProvider
          vid="banner"
          :name="$t('panel.settings.banner')"
          :rules="{ required: id ? false : true, image: true }"
          v-slot="{ validate, errors }"
          tag="div"
        >
          <label for="banner" class="mb-5">{{ $t("panel.settings.banner") }}</label>
          <div class="row">
            <div v-if="id" class="col-4">
              <img
                :src="$config.API_URL + 'uploads/settings/' + banner"
                class="img-fluid"
                alt="Banner"
              />
            </div>
            <div :class="id ? 'col-8' : 'col-12'">
              <input
                id="banner"
                class="form-control form-control-sm rounded-0"
                :placeholder="$t('panel.settings.banner')"
                type="file"
                name="banner"
                @change="validate"
                @input="$emit('update:banner', $event.target.files[0])"
              />
            </div>
          </div>
          <span class="mt-5 d-block text-danger">{{ errors[0] }}</span>
...
export default {
  ...
  props: ["banner","logo", "mobile_logo", "favicon", "id"],
};
</script>